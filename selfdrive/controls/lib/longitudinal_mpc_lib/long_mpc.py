...
class LongitudinalMpc:
  ...
  def update(self, carstate, radarstate, model, v_cruise, x, v, a):
    ...
    stopline = (model.stopLine.x + 4.0) * np.ones(N+1) if stopping else 400 * np.ones(N+1)
    x = (x[N] + 6.0) * np.ones(N+1)

    if self.status and not self.on_stopping:
      x_obstacles = np.column_stack([lead_0_obstacle, lead_1_obstacle, cruise_obstacle])
    elif x[N] > 30 and stopline[N] < 30 and self.v_ego < 16.0:
      self.on_stopping = False
      ...
      x_obstacles = np.column_stack([lead_0_obstacle, lead_1_obstacle, cruise_obstacle*2, (stopline*0.4)+(x*0.6)])
